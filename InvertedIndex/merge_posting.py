from Global_Index.read_index import read_index # importamos funcion para leer un bloque de indice especifico
from Global_Index.write_index import write_index  # importamos funcion para escribir un bloque de indice especifico
import sys
import json
import time
import math
...
def isFull(index: dict, len1: int,
           len2: int) -> bool:  # evalua si un dict se llenó (considerando longitud de los que dict's que se hacen merge)
    while len1 > 50000:
        len1 -= 30000
    while len2 > 50000:
        len2 -= 30000
...
def Merge1(index1: int, index2: int, ruta_origen: str,
           ruta_destino: str) -> None:  # index1: extremo izquierdo , index2: extremo derecho
    ...
    if mitad_llena == 1:  # falta escribir elementos de la segunda mitad
        ...
        while contador <= index2:
            while not isBlockFull:
                ...
                isBlockFull = isFull(result, len2,
                                     len2)  # verifica si bloque se llenó (solo compara con la longitud de bloque que está añadiendo)
        ...
        while contador <= index2:
            ...
            while not isBlockFull:
                ...
                isBlockFull = isFull(result, len1,
                                     len1)  # verifica si bloque se llenó (solo compara con la longitud de bloque que está añadiendo)
